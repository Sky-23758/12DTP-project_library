...
def login():
    ...
    if request.method == 'POST':
        ...
        db = get_db()
        cursor = db.execute("SELECT id, username, real_name, email, password, role FROM Users WHERE username = ?", (username,))
...
def borrow():
    ...
    if request.method == 'POST':
        ...
        db = get_db()
        cursor = db.execute("SELECT username, email FROM Users WHERE id = ?", (borrower_id,))
        user_data = cursor.fetchone()
...
def edit_borrow(id):
    ...
    borrow_date = datetime.strptime(borrow['borrow_date'], '%Y-%m-%d') if borrow['borrow_date'] else None
    return_date = datetime.strptime(borrow['return_date'], '%Y-%m-%d') if borrow['return_date'] else None
